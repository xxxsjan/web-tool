<template>
  <div class="flex flex-col justify-center items-center gap-4">
    <div class="card w-[90vw] max-w-3xl bg-base-100 shadow-xl"> <!-- 增加响应式宽度 -->
      <div class="card-body space-y-4">
        <h2 class="card-title text-2xl text-gray-700">图片转ICO工具</h2> <!-- 增强标题样式 -->
        ...
        <div class="bg-gray-50 p-4 rounded-lg border border-dashed border-gray-200">
          <div class="flex flex-col md:flex-row gap-4 items-center">
            <div class="form-control w-full max-w-xs">
              ...
              <input type="file" class="file-input file-input-bordered file-input-primary w-full" accept="image/*"
                @change="fileChange" />
            </div>
            ...
            <div class="form-control w-full max-w-xs">
              ...
              <select v-model="outputSize" class="select select-bordered select-primary w-full">
                <option v-for="size in [16, 32, 48, 64]" :value="size">{{ size }}x{{ size }}</option>
              </select>
            </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div v-for="(item, index) in resultArr" :key="index"
              ...
              <div class="flex items-center gap-4">
                <img :src="item.dataURL" class="w-16 h-16 object-contain rounded-lg border border-gray-200 p-1" />
                <div class="flex-1">
                  <p class="font-medium text-gray-700">{{ item.iconName }}</p>
                  <p class="text-sm text-gray-500">{{ outputSize }}x{{ outputSize }}像素</p>
                </div>
                <button @click="downCanvas(item)" class="btn btn-sm btn-ghost text-blue-500 hover:bg-blue-50">
